...
from add_data_to_solr.cy_solr_local.search_conf import ServerEnvironmentType
...
class Results(object):
    ...
    def show(self):
        print("found match docs:%s" % self.hits)
        for doc in self.docs:
            print("============= doc =============")
            for k, v in doc.items():
                print("%s:%s" % (k, v))
        if self.highlighting:
            print("============= highlighting info =============")
            for doc_id, info in self.highlighting.items():
                print("============= highlighting %s =============" % doc_id)
                for hl_field, hl_segments in info.items():
                    for hl_segment in hl_segments:
                        print("hl_field=%s, value=%s" % (hl_field, hl_segment))
        if self.facets:
            print("============= facets info =============")
            for facet_field, facet_info in self.facets["facet_fields"].items():
                print("============= facet_field %s =============" % facet_field)
                name_num_tuple_list = zip(facet_info[0::2], facet_info[1::2])
                for name_num_tuple in name_num_tuple_list:
                    print("%s:%s" % name_num_tuple)
...
class ZooKeeper(object):
    ...
    def __init__(self, zkServerAddress, timeout=15, max_retries=-1, kazoo_client=None):
        ...
        def updateClusterState(data, *args, **kwargs):
            if not data:
                LOG.warning("No cluster state available: no collections defined?")
            else:
                self.collections.update(json.loads(data.decode('utf-8')))
...
class SolrContext(object):
    def __init__(self):
        self.zookeeper_host = "zk_solr_1:2181,zk_solr_2:2181"

class SolrContext_md4(object):
    def __init__(self):
        self.zookeeper_host = "md4:2181"

class SolrTestContext(object):
    def __init__(self):
        self.zookeeper_host = "rd1:2181,rd2:2181"


class SolrHelper(object):
    """
    Sorl操作初始化封装
    PS: Testcase的特殊处理，保证testcase不抛异常
    """

    def __init__(self, server_type):
        assert ServerEnvironmentType.is_valid_server(server_type)
        is_online = ServerEnvironmentType.is_online_server(server_type)
        self.is_online = is_online
        self.server_type = server_type
        # testcase的特殊处理
        if ServerEnvironmentType.is_testcase(server_type):
            self.context = None
            self.zookeeper = None
            return

        self.context = SolrContext() if is_online else SolrTestContext()
        if server_type == "md4":
            self.context = SolrContext_md4()

        self.zookeeper = ZooKeeper(self.context.zookeeper_host)

    def get_solr(self, collname):
        # testcase的特殊处理
        if ServerEnvironmentType.is_testcase(self.server_type):
            return None
        if self.server_type == "md4":
            collname = "solr_" + collname
        else:
            collname = collname if self.is_online else "%s_%s" % (self.server_type, collname)

        if collname not in self.zookeeper.collections.keys():
            raise SolrError("Unknown collection: %s" % collname)
        return SolrCloud(self.zookeeper, collname)
