...
LOG = logging.getLogger('default_err')
h = NullHandler()
LOG.addHandler(h)
...
class Solr(object):
    ...
    def search(self, **kwargs):
        ...
        response = self._select(params, handler=None)
...
class ZooKeeper(object):
    ...
    def __init__(self, zkServerAddress, custom_collections=None, zkClientTimeout=15, zkClientConnectTimeout=15):
        ...
        if custom_collections:
            for collection_name in custom_collections:
                self.zk.DataWatch('/collections/%s/state.json' % collection_name)(watchClusterState)
